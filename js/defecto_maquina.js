(() => {
    if (!window.AppRouter || window.AppRouter.currentView !== 'defecto_maquina') {
        return;
    }
...
const { useState, useEffect, useMemo, useCallback } = React;
        ...
        const DEFECT_NAME_MAP = DEFECT_COLUMNS.reduce((acc, name) => {
            acc[normalizeDefectName(name)] = name;
            return acc;
        }, {});
        ...
        const isDefectColumn = (columnName) => {
            ...
            return Boolean(DEFECT_NAME_MAP[normalizedName]);
        };

        const getCanonicalDefectName = (columnName) => {
            const normalizedName = normalizeDefectName(columnName);
            return DEFECT_NAME_MAP[normalizedName] || extractDefectNameFromLabel(columnName);
        };
        ...
        function App() {
            ...
            const extractDefectName = (colName) => {
                return getCanonicalDefectName(colName);
            };

            const canonicalDefectColumns = useMemo(() => {
                if (rawData.length === 0) return [];
                const availableColumns = Object.keys(rawData[0]);
                const defectCols = availableColumns.filter(isDefectColumn);
                console.log('Columnas de defectos canónicas:', defectCols.length, defectCols);
                return defectCols;
            }, [rawData]);
            ...
            const defectsByMachine = useMemo(() => {
                ...
                filteredData.forEach(row => {
                    ...
                    if (!result[maquina]) {
                        ...
                        canonicalDefectColumns.forEach(def => result[maquina][def] = 0);
                    }
                    
                    canonicalDefectColumns.forEach(defect => {
                ...
                return result;
            }, [filteredData, canonicalDefectColumns]);
            ...
            const activeDefects = useMemo(() => {
                ...
                return canonicalDefectColumns.filter(d => active.has(d));
            }, [defectsByMachine, canonicalDefectColumns]);

            // Calcular totales por defecto
            const totalsByDefect = useMemo(() => {
                const totals = {};
                canonicalDefectColumns.forEach(defect => totals[defect] = 0);
                totals.total = 0;

(() => {
    ...
    root.innerHTML = `
        <div class="rt-app">
            ...
            <div class="rt-main">
                ...
                <main class="rt-content">
                    ...
                    <section class="view" id="rt-consulta-view">
                        <div class="rt-layout">
                            <div class="rt-alert" id="rt-consulta-alert"></div>
                            <div style="display:flex; justify-content:flex-end;">
                                <button type="button" class="rt-button secondary" id="rt-find-dups-btn">🧹 Detectar duplicados (SRV)</button>
                            </div>
                            <div class="rt-card">
            ...
            <div class="rt-modal-overlay" id="rt-search-modal">
                <div class="rt-modal-content">
                    ...
                    <div class="rt-modal-body">
                        <div class="rt-alert" id="rt-search-alert"></div>
            ...
            <div class="rt-modal-overlay" id="rt-edit-modal">
                <div class="rt-modal-content rt-modal-content--large">
                    ...
                    <div class="rt-modal-body rt-modal-body--scroll">
                        <div class="rt-alert" id="rt-edit-alert"></div>
                        <form id="rt-edit-form">
                            ...
                            <div class="rt-form-grid" style="grid-template-columns: repeat(5, 1fr); gap: 10px; margin-bottom: 20px;">
                                ...
                                <div class="rt-field"><label>OP</label><input type="text" id="rt-edit-op" data-numeric="true" readonly title="Campo de identificación: no editable" style="background:#f1f5f9;cursor:not-allowed;"></div>
                                <div class="rt-field"><label>Partida</label><input type="text" id="rt-edit-partida" data-numeric="true" readonly title="Campo de identificación: no editable" style="background:#f1f5f9;cursor:not-allowed;"></div>
                                <div class="rt-field"><label>Color</label><input type="text" id="rt-edit-color" class="rt-uppercase"></div>
                                <div class="rt-field"><label>Cod. Art.</label><input type="text" id="rt-edit-cod-articulo" data-numeric="true" readonly title="Campo de identificación: no editable" style="background:#f1f5f9;cursor:not-allowed;"></div>
                                <div class="rt-field"><label>Descripción</label><input type="text" id="rt-edit-descripcion" class="rt-uppercase"></div>
                                <div class="rt-field"><label>Rollo</label><input type="text" id="rt-edit-n-rollo" data-numeric="true" readonly title="Campo de identificación: no editable" style="background:#f1f5f9;cursor:not-allowed;"></div>
                                ...
                                <div class="rt-field"><label>Merma (kg)</label><input type="text" id="rt-edit-merma" data-decimal="true"></div>
                            ...
                            <div class="rt-field" style="margin-bottom: 20px;"><label>Comentario evaluación</label><textarea id="rt-edit-comentario" class="rt-uppercase" placeholder="OPCIONAL..."></textarea></div>
            ...
            <!-- MODAL DE DETECCIÓN / LIMPIEZA DE DUPLICADOS (SRV) -->
            <div class="rt-modal-overlay" id="rt-dup-modal">
                <div class="rt-modal-content rt-modal-content--large">
                    <div class="rt-modal-header">
                        <h3>Duplicados de terceros (SRV)</h3>
                        <button type="button" class="rt-modal-close-icon" id="rt-dup-close-x">&times;</button>
                    </div>
                    <div class="rt-modal-body rt-modal-body--scroll">
                        <div class="rt-alert" id="rt-dup-alert"></div>
                        <div id="rt-dup-summary" style="margin-bottom:12px; font-weight:600; color:#475569;">Buscando duplicados...</div>
                        <div class="rt-table-wrap">
                            <table class="rt-table rt-table--compact">
                                <thead>
                                    <tr><th>OP</th><th>Partida</th><th>Rollo</th><th>Cod.Art</th><th>Fecha</th><th>Inspector</th><th>Peso</th><th>Fila</th><th>Estado</th></tr>
                                </thead>
                                <tbody id="rt-dup-tbody"></tbody>
                            </table>
                        </div>
                    </div>
                    <div class="rt-modal-footer">
                        <button type="button" class="rt-button secondary" id="rt-dup-cancel-btn">Cerrar</button>
                        <button type="button" class="rt-button primary" id="rt-dup-remove-btn" style="display:none; background:#dc2626; border-color:#dc2626;">Eliminar duplicados</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    const elements = {
        ...
        dupModal: document.getElementById("rt-dup-modal"),
    };
    ...
    renderConsultaTable();
    loadCatalogs();
    // La base completa se carga bajo demanda (al usar 🔍 Buscar o abrir Consulta),
    // no en el arranque, para no descargar todo el sheet a usuarios que no lo necesitan.
    applySidebarState();
    ...
    function bindEvents() {
        ...
        document.getElementById("rt-search-main-btn").addEventListener("click", () => {
            hideAlert("rt-search-alert");
        ...
        // Eventos Modal Duplicados
        document.getElementById("rt-find-dups-btn").addEventListener("click", openDuplicatesModal);
        document.getElementById("rt-dup-cancel-btn").addEventListener("click", () => elements.dupModal.classList.remove("active"));
        document.getElementById("rt-dup-close-x").addEventListener("click", () => elements.dupModal.classList.remove("active"));
        document.getElementById("rt-dup-remove-btn").addEventListener("click", removeDuplicates);
    ...
    async function searchAndPopulateMainData() {
        ...
        if (!op) {
            showAlert("rt-search-alert", "Aviso", "Debes ingresar una OP.", false);
            return;
        }
        hideAlert("rt-search-alert");
        ...
        try {
            // Pide al servidor solo las filas SRV de esta OP (filtrado en el servidor),
            // en vez de descargar toda la base.
            const url = `${SOURCE_APPS_SCRIPT_URL}?action=getTercerosByOp&op=${encodeURIComponent(op)}&nocache=${Date.now()}`;
            const response = await fetch(url, { cache: "no-store" });
            const payload = await response.json();
            const allRecords = (payload && payload.success && Array.isArray(payload.data)) ? payload.data : [];

            if (!allRecords.length) {
                showAlert("rt-search-alert", "Aviso", `No se encontraron registros de terceros (SRV) para la OP ${op}.`, false);
                return;
            }
            ...
            if (!match) {
                showAlert("rt-search-alert", "Aviso", `No se encontró registro para la OP ${op} y Partida ${partida} en máquinas SRV.`, false);
                return;
            }
            ...
            showAlert("rt-search-alert", "Error", "Error al conectar con el servidor: " + error.message, false);
        } finally {
            btn.disabled = false;
    ...
    function removeRoll(index) {
        const roll = state.sessionRolls[index];
        const rolloLabel = roll ? (roll.detalle && roll.detalle.nRollo) || "" : "";
        if (!window.confirm(`¿Quitar el rollo ${rolloLabel || index + 1} de la lista?`)) {
            return;
        }
    ...
    function mapSheetRecordsToRolls(records, sampleRow) {
        ...
        const cols = {
            ...
            comentario: findColumnName(allCols, ['Comentarios', 'Comentario', 'Comentarios evaluacion', 'Comentario Evaluacion']),
        ...
        return (records || []).map(r => {
            ...
            return {
                ...
                detalle: {
                    ...
                    comentarioEvaluacion: String(cols.comentario ? (r[cols.comentario] || "") : ""),
                },
                user: {
    ...
    function openEditModal(editIndex, idUnico) {
        ...
        if (!roll) {
            showAlert("rt-consulta-alert", "Aviso", "No se encontró el registro para editar en la vista actual.", false);
            return;
        }

        hideAlert("rt-edit-alert");
        ...
        document.getElementById("rt-edit-merma").value = roll.detalle.merma || "";
        document.getElementById("rt-edit-comentario").value = roll.detalle.comentarioEvaluacion || "";
    ...
    function addDefectToEdit() {
        ...
        if (!defectName || !occurrenceText) {
            showAlert("rt-edit-alert", "Aviso", "Selecciona un defecto y registra la ocurrencia.", false);
            return;
        }

        const occurrence = Number(occurrenceText);
        if (!Number.isFinite(occurrence) || occurrence <= 0) {
            showAlert("rt-edit-alert", "Aviso", "La ocurrencia debe ser mayor que cero.", false);
            return;
        }
        hideAlert("rt-edit-alert");
    ...
    async function saveEditedRoll() {
        ...
        const updatedRoll = {
            ...
            detalle: {
                ...
                merma: document.getElementById("rt-edit-merma").value.trim(),
                comentarioEvaluacion: document.getElementById("rt-edit-comentario").value.trim().toUpperCase(),
            },
        ...
        if (!updatedRoll.datosPrincipales.op || !updatedRoll.detalle.nRollo) {
            showAlert("rt-edit-alert", "Aviso", "OP y Nro de Rollo son obligatorios.", false);
            return;
        }
        hideAlert("rt-edit-alert");
        ...
        try {
            ...
            hideAlert("rt-edit-alert");
            showAlert("rt-consulta-alert", "Éxito", "Registro actualizado correctamente.", true);

        } catch (error) {
            showAlert("rt-edit-alert", "Error", error.message || "No se pudo guardar la información.", false);
        } finally {
            btn.disabled = false;
            btn.textContent = originalText;
        }
    ...
    // --- DETECCIÓN / LIMPIEZA DE DUPLICADOS (SRV) ---

    async function openDuplicatesModal() {
        elements.dupModal.classList.add("active");
        hideAlert("rt-dup-alert");
        const tbody = document.getElementById("rt-dup-tbody");
        const summary = document.getElementById("rt-dup-summary");
        const removeBtn = document.getElementById("rt-dup-remove-btn");

        removeBtn.style.display = "none";
        summary.textContent = "Buscando duplicados...";
        tbody.innerHTML = `<tr><td colspan="9" class="rt-empty">Analizando la base...</td></tr>`;

        try {
            const data = await fetchBackendJson("findDuplicates");
            renderDuplicates(data);
        } catch (error) {
            summary.textContent = "";
            tbody.innerHTML = `<tr><td colspan="9" class="rt-empty" style="color:#ef4444;">Error: ${escapeHtml(error.message)}</td></tr>`;
        }
    }
    ...
    function renderDuplicates(data) {
        ...
        if (!groups.length) {
            summary.textContent = "✓ No se encontraron duplicados de terceros (SRV).";
            tbody.innerHTML = `<tr><td colspan="9" class="rt-empty">Todo limpio.</td></tr>`;
            removeBtn.style.display = "none";
            return;
        }

        summary.textContent = `Se encontraron ${data.totalExtraRows} fila(s) duplicada(s) en ${data.totalGroups} grupo(s). Se conservará la fila más reciente de cada grupo (verde) y se eliminarán las demás (rojo).`;

        tbody.innerHTML = groups.map((group, gi) => {
            const groupHeader = `<tr style="background:#eef2f7;"><td colspan="9"><strong>Grupo ${gi + 1}</strong> — ${escapeHtml(group.key)} (${group.count} filas)</td></tr>`;
            const rowsRendered = group.rows.map((r, ri) => {
                const isKept = ri === group.rows.length - 1;
                const bg = isKept ? "background:#ecfdf3;" : "background:#fef2f2;";
                const estado = isKept
                    ? '<span style="color:#15803d;font-weight:600;">Conservar</span>'
                    : '<span style="color:#b91c1c;font-weight:600;">Eliminar</span>';
                return `
                    <tr style="${bg}">
                        <td>${escapeHtml(r.op)}</td>
                        <td>${escapeHtml(r.partida)}</td>
                        <td><strong>${escapeHtml(r.rollo)}</strong></td>
                        <td>${escapeHtml(r.codArt)}</td>
            ...
            return groupHeader + rowsRendered;
        }).join("");

        removeBtn.textContent = `Eliminar ${data.totalExtraRows} duplicado(s)`;
        removeBtn.style.display = "inline-flex";
    }

    async function removeDuplicates() {
        if (!window.confirm("Esta acción eliminará de forma permanente las filas marcadas en rojo. ¿Continuar?")) {
            return;
        }
        ...
        try {
            const response = await fetch(SOURCE_APPS_SCRIPT_URL, {
                method: "POST",
                headers: { "Content-Type": "text/plain" },
                body: JSON.stringify({ action: "removeDuplicates" }),
            });
            ...
            const data = await response.json();
            if (!data.success) {
                throw new Error(data.error || "No se pudieron eliminar los duplicados.");
            }

            consultaAllRowsCache = null;
            showAlert("rt-dup-alert", "Éxito", data.message || `${data.deletedRows} fila(s) eliminada(s).`, true);

            // Volver a detectar para confirmar que quedó limpio
            const refreshed = await fetchBackendJson("findDuplicates");
            renderDuplicates(refreshed);

            // Refrescar la tabla de consulta detrás del modal
            await fetchConsultaData();
        } catch (error) {
            showAlert("rt-dup-alert", "Error", error.message || "No se pudieron eliminar los duplicados.", false);
        } finally {

(() => {
        ...
        <div class="rt-app">
            <aside class="rt-sidebar">
                <div class="rt-sidebar-header">
                    <div class="rt-logo"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/></svg></div>
                    <h1 class="rt-sidebar-title">ASEGURAMIENTO DE LA  CALIDAD TEXTIL</h1>
                    <a class="rt-sidebar-back" href="${window.AppRouter.href("home")}">Volver al inicio</a>
                </div>
                <nav class="rt-nav">
                    <button type="button" class="rt-nav-button active" data-nav-view="login"><span class="rt-nav-icon">PF</span><span>Perfil</span></button>
                    <button type="button" class="rt-nav-button" data-nav-view="registro" id="rt-nav-registro" style="display:none;"><span class="rt-nav-icon">RG</span><span>Registro</span></button>
                    <button type="button" class="rt-nav-button" data-nav-view="consulta" id="rt-nav-consulta" style="display:none;"><span class="rt-nav-icon">CN</span><span>Consulta</span></button>
                </nav>
                ...
                <main class="rt-content">
                        ...
                        <div class="rt-login-shell">
                            <div class="rt-login-card">
                                <div class="rt-login-header">
                                    ...
                                    <h1>Registro Insp T-Cruda terceros</h1>
                                </div>
                    ...
                    <section class="view" id="rt-registro-view">
                        <div class="rt-layout">
                            ...
                            <div class="rt-card">
                                ...
                                <div class="rt-card-body">
                                    <div class="rt-form-grid rt-form-grid-compact">
                                        ...
                                        <div class="rt-field"><label for="rt-titulo-hilo">Título de Hilo</label><input type="text" id="rt-titulo-hilo" class="rt-uppercase"></div>
                                        <div class="rt-field"><label for="rt-lote-hilo">Lote hilo</label><input type="text" id="rt-lote-hilo" class="rt-uppercase"></div>
                                        <div class="rt-field"><label for="rt-lote-spandex">Lote spandex</label><input type="text" id="rt-lote-spandex" class="rt-uppercase"></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="rt-card">
                                <div class="rt-card-header" style="background:#e0e7ff;"><h3 id="rt-rolls-header">Lista de rollos a guardar (0)</h3></div>
                    ...
                    <div class="rt-modal-body rt-modal-body--scroll">
                        <form id="rt-edit-form">
                            ...
                            <div class="rt-form-grid" style="grid-template-columns: repeat(5, 1fr); gap: 10px; margin-bottom: 20px;">
                                ...
                                <div class="rt-field"><label>Título de Hilo</label><input type="text" id="rt-edit-titulo-hilo" class="rt-uppercase"></div>
                                <div class="rt-field"><label>Lote Hilo</label><input type="text" id="rt-edit-lote-hilo" class="rt-uppercase"></div>
                                <div class="rt-field"><label>Lote Lycra</label><input type="text" id="rt-edit-lote-spandex" class="rt-uppercase"></div>
                                <div class="rt-field"><label>Máquina</label><input type="text" id="rt-edit-maquina" class="rt-uppercase"></div>
                            </div>

                            <div class="rt-card" style="border: 1px solid #c7d2fe; background: #f5f8ff; margin-bottom: 0;">
                                <div class="rt-card-header" style="background: #e0e7ff; padding: 8px 16px;">
                                    <h4 style="margin:0; font-size: 13px; color: #312e81;">Gestión de Defectos</h4>
                                </div>
    ...
    async function fetchBackendJson(action) {
        const url = `${WEB_APP_URL}?action=${encodeURIComponent(action)}&nocache=${Date.now()}`;
    ...
    async function loadCatalogsFromSheets() {
        const [baseRows, tercerosRows, customerRows] = await Promise.all([
            loadFirstAvailableSheetRows([SHEET_NAME]),
            loadFirstAvailableSheetRows(["terceros", "TERCEROS", "PERSONAL", "SERVICIO"]).catch(() => []),
            loadFirstAvailableSheetRows(["CUSTOMER", "customer"]).catch(() => []),
        ]);

        return {
            defectos: buildDefectCatalogFromRows(baseRows),
            ...
            customer: normalizeSimpleValues(
                collectUniqueValuesFromRows(customerRows, ["Cliente", "Customer"]).concat(
                    collectUniqueValuesFromRows(baseRows, ["Cliente", "Customer"])
                )
            ),
        };
    }
    ...
    async function loadCatalogs() {
        ...
        populateDefectOptions([], "Seleccionar...");
        ...
        try {
            ...
            if (!hasUsableCatalogs(catalogs)) {
                ...
                catalogs = {
                    defectos: normalizeDefectCatalog((catalogs && catalogs.defectos) || []).length
                        ? normalizeDefectCatalog(catalogs.defectos)
                        : normalizeDefectCatalog(sheetCatalogs.defectos),
            ...
            state.defectCatalog = normalizeDefectCatalog(catalogs.defectos);
    ...
    function handleLogin(event) {
        ...
        elements.userDetail.textContent = `Turno: ${turno} | ${formatShortDate(new Date(fecha))}`;
    ...
    function updateNavigationVisibility(isLoggedIn) {
        ...
        elements.navConsulta.style.display = isLoggedIn ? "flex" : "none";
    }
    ...
    function getMainData() {
        return {
            ...
            tituloHilo: document.getElementById("rt-titulo-hilo").value.trim().toUpperCase(),
    ...
    async function searchAndPopulateMainData() {
        ...
        try {
            ...
            const dataToSet = {
                ...
                "rt-titulo-hilo": matchLotCols.tituloHilo ? (match[matchLotCols.tituloHilo] || "") : "",
                "rt-lote-hilo": matchLotCols.loteHilo ? (match[matchLotCols.loteHilo] || "") : "",
                "rt-lote-spandex": matchLotCols.loteSpandex ? (match[matchLotCols.loteSpandex] || "") : "",
            };
    ...
    async function submitAllRolls() {
        ...
        try {
            ...
            state.filteredRolls = [...state.globalRolls];
            persistRolls();
    ...
    function openEditModal(editIndex, idUnico) {
        ...
        inspectorSelect.innerHTML = (state.personalCatalog || [])
    ...
    async function saveEditedRoll() {
        ...
        const updatedRoll = {
            ...
            datosPrincipales: {
                ...
                tituloHilo: document.getElementById("rt-edit-titulo-hilo").value.trim().toUpperCase(),
                loteHilo: document.getElementById("rt-edit-lote-hilo").value.trim().toUpperCase(),
                loteSpandex: document.getElementById("rt-edit-lote-spandex").value.trim().toUpperCase(),
            },
            detalle: {
        ...
        try {
            ...
            // Re-ordenar y filtrar
            persistRolls();
            
            // Refrescar datos de la nube para ver los cambios aplicados

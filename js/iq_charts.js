...
// El bloque de filtros de la página principal se reubica dentro del modal de
// Pareto mientras está abierto: así se evita duplicar IDs/listeners y se
// garantiza que la gráfica use exactamente los mismos datos y estado que la
// página principal. Al cerrar, los filtros regresan a su lugar original.
let _iqFiltersPlaceholder = null;

function iqMoveFiltersIntoModal() {
    const filters = document.querySelector('.iq-filters');
    const mount   = document.getElementById('pareto-filters-mount');
    if (!filters || !mount || mount.contains(filters)) return;

    if (!_iqFiltersPlaceholder) {
        _iqFiltersPlaceholder = document.createElement('div');
        _iqFiltersPlaceholder.className = 'hidden';
    }
    // Marca el lugar original para poder restaurarlo exactamente.
    filters.parentNode.insertBefore(_iqFiltersPlaceholder, filters);
    mount.appendChild(filters);
    filters.classList.add('iq-filters-in-modal');
}

function iqRestoreFilters() {
    const filters = document.querySelector('.iq-filters');
    if (!filters || !_iqFiltersPlaceholder || !_iqFiltersPlaceholder.parentNode) return;

    _iqFiltersPlaceholder.parentNode.insertBefore(filters, _iqFiltersPlaceholder);
    _iqFiltersPlaceholder.remove();
    filters.classList.remove('iq-filters-in-modal');
}

function iqOpenParetoModal(records, mode) {
    const modal = document.getElementById('pareto-modal');
    if (!modal) return;
    iqMoveFiltersIntoModal();

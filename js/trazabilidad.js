(() => {
    if (!window.AppRouter || window.AppRouter.currentView !== 'trazabilidad') {
        return;
    }
...
const { useState, useEffect, useMemo, useRef } = React;
        ...
        const syncWithGoogleSheet = async (rows) => {
            ...
            console.log('[sync] URL:', APPS_SCRIPT_URL.slice(0, 60) + '...');
            ...
            console.log('[sync] Petición enviada (respuesta opaca con no-cors)');
        ...
        // --- ICONOS SVG ---
        const IconUpload = () => (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="á21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>);
        const IconFileSpreadsheet = () => (<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-green-600"><path d="á14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><path d="á8 13h2"/><path d="á8 17h2"/><path d="á14 13h2"/><path d="á14 17h2"/></svg>);
        const IconPrinter = () => (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="á6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>);
        ...
        // --- TIPO CATEGORÍA: INSPECCION -> Insp, LIBERADO -> Lib, cualquier otro/vacío -> "" ---
        const formatTipoCategoria = (row) => {
            const raw = String(
                row["Tipo categoría"] || row["Tipo categoria"] ||
                row["Tipo Categoría"] || row["Tipo Categoria"] || ""
            ).trim().toUpperCase();
            if (raw === "INSPECCION" || raw === "INSPECCIÓN") return "Insp";
            if (raw === "LIBERADO") return "Lib";
            return "";
        };
        ...
        // --- DATOS DE EJEEMPLO ---
        const DEMO_DATA = [
            { "Año": 2025, "Mes": "Ago", "OP": "39404", "Partida": "9", "Rollo": 1720, "Cod. Art.": "29598", "Descripción": "JERSEY 40/1 VI COP ORG...", "Color": "BLACK", "Peso": 20.95, "Máquina": "JáO-015", "Hueco Con Cordon": 2, "Malla Rota": 4 },
            { "Año": 2025, "Mes": "Ago", "OP": "39404", "Partida": "9", "Rollo": 1698, "Cod. Art.": "29598", "Descripción": "JERSEY 40/1 VI COP ORG...", "Color": "BLACK", "Peso": 21.4, "Máquina": "JáO-015", "Cordon": 1, "Hilo Irregular": 1 },
            { "Año": 2025, "Mes": "Ago", "OP": "39404", "Partida": "2", "Rollo": 867, "Cod. Art.": "29598", "Descripción": "JERSEY...", "Color": "BLACK", "Peso": 9.4, "Máquina": "Já-013", "Hueco Con Cordon": 3, "Rotura De Aguja": 1 },
            { "Año": 2025, "Mes": "Ago", "OP": "39404", "Partida": "2", "Rollo": 868, "Cod. Art.": "29598", "Descripción": "JERSEY...", "Color": "BLACK", "Peso": 20.75, "Máquina": "Já-013", "Hueco Con Cordon": 3, "Falla De Lycra": 1, "Escapes De Lycra": 1 },
            { "Año": 2025, "Mes": "Ago", "OP": "39404", "Partida": "2", "Rollo": 869, "Cod. Art.": "OTRO-ART", "Descripción": "JERSEY CON OTRO TIPO DE HILO...", "Color": "BLACK", "Peso": 2.5, "Máquina": "Já-014", "Hueco Con Cordon": 1, "Cordon": 2 }
        ];
        ...
        function App() {
            ...
            const isValidOP = /^\d{3,5}$/.test(String(selectedOP || "").trim());
            ...
            // Columnas de defectos visibles (ocultar las últimas 5 columnas sin datos;
            // se subió de 4 a 5 para compensar el ancho de la nueva columna "Tipo categoría")
            const visibleDefectColumns = useMemo(() => {
                ...
                // Encontrar las últimas 5 columnas sin datos (recorriendo desde el final)
                const emptyColumnsFromEnd = [];
                for (let i = DEFECT_COLUMNS.length - 1; i >= 0 && emptyColumnsFromEnd.length < 5; i--) {
                    if (!columnsWithData.has(DEFECT_COLUMNS[i])) {
                        emptyColumnsFromEnd.push(DEFECT_COLUMNS[i]);
                    }
                }

                // Retornar columnas excluyendo las últimas 5 vacías
            ...
            const fechaAuditoria = useMemo(() => {
                ...
                const parseFechaConAnio = (diaVal, anioVal) => {
                    ...
                    if (typeof diaVal === 'string') {
                        // Formato: "dd-áon" (ej: "11-Jan", "3-Feb")
            ...
            const handleFileUpload = async (e) => {
                ...
                reader.onload = async (evt) => {
                    try {
                        ...
                        // Nombre de la columna OP para filtrar filas sin datos válidos
                        const opColName = headers.find(h => String(h || '').trim().toUpperCase() === 'OP');

                        // Filtrar filas de resumen/totales: por valor "TOTAL" en cualquier columna clave
                        // o por OP vacío (las filas TOTAL siempre tienen OP vacío)
                        const jsonData = jsonDataRaw.filter(row => {
                            // Excluir si alguna celda contiene exactamente "TOTAL"
                            const hasTotalCell = Object.values(row).some(v => String(v || '').toUpperCase().trim() === 'TOTAL');
                            if (hasTotalCell) return false;
                            // Excluir si OP está vacío (filas de suma/encabezado)
                        ...
                        // Mapa de nombres alternativos de columnas del Excel al nombre canónico del sheet
                        ...
                        // Eliminar columnas sin encabezado (__EMPTY*) y aplicar aliases de columnas
                        ...
                        const estimatedTime = Math.ceil(cleanedData.length / 500) + 5;
                        ...
                        try {
                            ...
                            const normalizedJsonData = cleanedData.map(row => normalizeColumnNames(row));
                            ...
                            showModalSuccess(
                                `Se procesaron ${cleanedData.length} filas correctamente`,
            ...
            return (
                <div className="min-h-screen">
                    {/* PANEL DE CONTROL (NO IMPRESIBLE) */}
                        ...
                        <div className="max-w-7xl mx-auto space-y-6">
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                                    ...
                                    <div className="flex gap-3 items-center flex-1 flex-col sm:flex-row">
                                        ...
                                        <div className="flex items-center gap-2 flex-1 sm:flex-none w-full sm:w-auto">
                                            ...
                                            <select 
                                                ...
                                                disabled={!isValidOP || availablePartidas.length === 0 || isLoading}
                                            >
                                ...
                                <div className="flex gap-3 justify-end lg:justify-start">
                                    ...
                                    <a href={window.AppRouter.href('home')} title="Volver al menú principal" className="flex items-center justify-center w-12 h-12 bg-[#001a54] text-white rounded-full hover:bg-[#1a3a7a] transition-colors shadow-sm" style={{textDecoration:'none',fontSize:'20px'}}>&#8592;</a>
                                </div>
                            </div>
                            ...
                            <div id="report-area" className="text-black">
                                        ...
                                        <tbody>
                                            ...
                                            {(() => {
                                                ...
                                                if (filteredData && filteredData.length > 0) {
                                                    ...
                                                    if (firstRow) {
                                                        const catKey = ['Categoria', 'Categoría'].find(k => firstRow[k] !== undefined && firstRow[k] !== '')
                                            || Object.keys(firstRow).find(k => k && k.toLowerCase().includes('categoria') && !k.toLowerCase().includes('tipo'));
                                ...
                                <div className="overflow-x-auto">
                                    <table className="report-table w-full border-collapse print:text-[9pt] font-mono whitespace-nowrap">
                                        <thead>
                                            <tr className="bg-gray-300 text-center print:bg-gray-300 align-bottom">
                                                ...
                                                <th className="border border-gray-400 px-1 py-2 font-bold text-sm print:text-[9pt]">Máquina</th>
                                                <th className="border border-gray-400 px-1 py-2 w-8 print:w-4 h-32 align-bottom">
                                                    <div className="flex h-full items-end justify-center pb-1">
                                                        <div className="vertical-text text-xs print:text-[8pt]">Tipo categoría</div>
                                                    </div>
                                                </th>
                                                ...
                                                <th className="border border-gray-400 px-1 py-2 bg-gray-300 font-bold min-w-[30px] print:min-w-0 text-sm print:text-[9pt]">TOT</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                ...
                                                return (
                                                    <React.Fragment key={codArt}>
                                                        {groupRows.map((row, index) => {
                                                            ...
                                                            const isLib = formatTipoCategoria(row) === "Lib";
                                                            return (
                                                                <tr key={`${codArt}-${index}`} className={`text-center hover:bg-gray-50 print:hover:bg-transparent ${isLib ? 'bg-gray-100 print:bg-gray-100' : ''}`}>
                                                                    <td className={`border border-gray-300 px-1 py-1 text-xs print:text-[9pt] font-semibold align-top ${isLib ? 'bg-gray-100 print:bg-gray-100' : 'bg-white'}`}>{showCodArt ? codArt : ""}</td>
                                                                    ...
                                                                    <td className="border border-gray-300 px-1 py-1 print:text-[9pt]">{String(row["Máquina"] || row["Maquina"] || "").toUpperCase().includes("SRV") ? "SRV" : (row["Máquina"] || row["Maquina"] || "")}</td>
                                                                    <td className="border border-gray-300 px-1 py-1 text-xs print:text-[8pt]">{formatTipoCategoria(row)}</td>
                                                                    {visibleDefectColumns.map((col) => {
                                                        ...
                                                        <tr className="bg-gray-300 font-bold text-center border-t border-b-2 border-gray-400 print:bg-gray-300">

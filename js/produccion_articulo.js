...
const { useState, useEffect, useMemo, useCallback } = React;
        ...
        function App() {
            ...
            const productionByArticle = useMemo(() => {
                ...
                rawData.forEach(row => {
                    const codArt = String(getRowValue(row, codArtColumn, ['Cod. Art.', 'Cod Art', 'CodArt', 'Codigo']) || '').trim();
                    ...
                    if (rolloId) {
                        result[codArt].rolloSet.add(rolloId);
                    } else {
                        const fallbackRollos = parseNumericValue(getRowValue(row, rollosColumn, ['Rollos', 'Cantidad'])) || 1;
                        result[codArt].rollos += fallbackRollos;
                    }

                    result[codArt].pesoKg += pesoKg;
            ...
            const handleExportExcel = async () => {
                try {
                    ...
                    // Encabezados
                    const headers = ["Nº", "Cod. Art.", "Descripción", "#Rollos", "%Part\nRollos", "Kg", "%Part\nkg", "Total Defectos", "Defecto/kg"];
                    ...
                    sortedArticles.forEach((article, index) => {
                        const percentageRollos = grandTotalRollos > 0 ? ((article.rollos / grandTotalRollos) * 100).toFixed(1) : 0;
                    ...
                    const footerRow = worksheet.addRow(['', '', 'TOTAL', grandTotalRollos, '100%', grandTotalKg, '100%', grandTotalDefectos, grandDefectosPorKg]);
                    ...
                    footerRow.getCell(6).numFmt = '#,##0.0';
                                    ...
                                    <table className="w-full table-fixed text-[10px] sm:text-xs lg:text-sm">
                                        <thead className="sticky top-0">
                                            <tr className="bg-gray-100">
                                                ...
                                                <th className="px-1.5 sm:px-2 py-2 text-left font-bold text-gray-700 border-b w-[46%] whitespace-nowrap">
                                                    Descripción
                                                </th>
                                                <th className="px-1 sm:px-1.5 py-2 text-center font-bold text-gray-700 border-b bg-blue-50 w-[6%]">
                                                    #Rollos
                                                </th>
                                                <th className="px-1 sm:px-1.5 py-2 text-center font-bold text-gray-700 border-b bg-blue-50 w-[7%] leading-tight">
                                                    <span className="block">%Part</span>
                                                    <span className="block">Rollos</span>
                                                </th>
                                            ...
                                            {sortedArticles.map((article, idx) => {
                                                ...
                                                return (
                                                    <tr key={article.codArt} className={`${rowBgClass} hover:bg-blue-50`}>
                                                        ...
                                                        <td className="px-1 sm:px-1.5 py-1.5 text-center font-bold text-blue-700 border-b bg-blue-50">
                                                            {formatInteger(article.rollos)}
                                                        </td>
                                                        <td className="px-1 sm:px-1.5 py-1.5 text-center font-medium text-blue-700 border-b bg-blue-50">
                                                            {formatPercent(percentageRollos)}
                                                        </td>
                                        ...
                                        <tfoot>
                                            <tr className="bg-gray-200 font-bold sticky bottom-0">
                                                ...
                                                <td className="px-1 sm:px-1.5 py-2 text-center text-blue-800 border-t bg-blue-100">
                                                    {formatInteger(grandTotalRollos)}
                                                </td>
                                                <td className="px-1 sm:px-1.5 py-2 text-center text-blue-800 border-t bg-blue-100">
                                                    100%
                                                </td>

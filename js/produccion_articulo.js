(() => {
    if (!window.AppRouter || window.AppRouter.currentView !== 'produccion_articulo') {
        return;
    }
...
const { useState, useEffect, useMemo, useCallback } = React;
        ...
        // ============================================
        const SHEET_ID = SOURCE_SHEET_ID; // ID del Google Sheet
        const SHEET_NAME = SOURCE_SHEET_NAME; // Nombre de la hoja
        // ============================================
        ...
        const getRowValueByPatterns = (row, patterns = []) => {
            ...
            for (const pattern of patterns) {
                ...
                const exactMatch = entries.find((entry) => entry.normalized === normalizedPattern);
                if (exactMatch && exactMatch.value !== undefined && exactMatch.value !== null && String(exactMatch.value).trim() !== '') {
                    return exactMatch.value;
                }

                const partialMatch = entries.find((entry) =>
                    entry.normalized.includes(normalizedPattern) ||
                    normalizedPattern.includes(entry.normalized)
                );
                if (partialMatch && partialMatch.value !== undefined && partialMatch.value !== null && String(partialMatch.value).trim() !== '') {
        ...
        const formatInteger = (value) => Math.round(Number(value || 0)).toLocaleString('es-PE');
        const formatKg = (value) => Number(value || 0).toLocaleString('es-PE', {
            minimumFractionDigits: 1,
            maximumFractionDigits: 1
        });
        const formatRatio = (value) => Number(value || 0).toLocaleString('es-PE', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
        });
        const formatPercent = (value) => `${Number(value || 0).toLocaleString('es-PE', {
            minimumFractionDigits: 1,
            maximumFractionDigits: 1
        })}%`;
        ...
        // Obtener fecha segura a partir de una fila: prioriza fecha completa, luego intenta usar columna 'Año' si falta el año
        const getSafeRowDate = (row) => {
            const rawDateStr = row['Día'] || row['Dia'] || row['Day'] || row['Fecha'] || '';
            let rowDate = parseSheetDate(rawDateStr);

            // Si la cadena original no contiene año (no hay 4 dígitos) y parseó con año por defecto (p.ej. 2001), intentar usar columna 'Año'
            const lacksYearInString = rawDateStr && !/\d{4}/.test(String(rawDateStr));
            if ((rowDate && rowDate.year === 2001 && lacksYearInString) || !rowDate) {
                // Buscar columna de año en la fila
                const yearCandidates = [row['Año'], row['Ano'], row['Year'], row['YEAR']];
                let year = null;
                for (const c of yearCandidates) {
                    if (c === undefined || c === null) continue;
                    const s = String(c).trim();
                    if (/^\d{4}$/.test(s)) { year = Number(s); break; }
                    // A veces viene como número (2026) directamente
                    if (!isNaN(Number(s)) && Number(s) > 1900 && Number(s) < 3000) { year = Number(s); break; }
                }

                if (year) {
                    // Intentar añadir el año a la cadena original y parsear
                    if (rawDateStr) {
                        const augmented = rawDateStr + '/' + year;
                        const parsedAug = parseSheetDate(augmented);
                        if (parsedAug) return parsedAug;
                    }

                    // Si no hay rawDateStr o el augmented falló, intentar construir desde campos separados
                    const dayCandidate = row['Día'] || row['Dia'] || row['Day'] || row['Dia.'] || row['Day.'];
                    const monthCandidate = row['Mes'] || row['MES'] || row['Mes.'] || row['Month'] || row['month'];
                    let day = null, month = null;
                    if (dayCandidate !== undefined && dayCandidate !== null) {
                        const d = parseInt(String(dayCandidate).replace(/[^0-9]/g, ''), 10);
                        if (!isNaN(d)) day = d;
                    }
                    if (monthCandidate) {
                        const mstr = String(monthCandidate).trim().toLowerCase();
                        const mesesMapLocal = {
                            'ene':0,'enero':0,'feb':1,'febrero':1,'mar':2,'marzo':2,'abr':3,'abril':3,'may':4,'mayo':4,'jun':5,'junio':5,'jul':6,'julio':6,'ago':7,'agosto':7,'sep':8,'sept':8,'septiembre':8,'oct':9,'octubre':9,'nov':10,'noviembre':10,'dic':11,'diciembre':11
                        };
                        // Soportar formatos numéricos (1-12)
                        const mnum = parseInt(mstr, 10);
                        if (!isNaN(mnum) && mnum >= 1 && mnum <= 12) month = mnum - 1;
                        else {
                            const key = mstr.substring(0,3);
                            if (mesesMapLocal[key] !== undefined) month = mesesMapLocal[key];
                            else if (mesesMapLocal[mstr] !== undefined) month = mesesMapLocal[mstr];
                        }
                    }

                    if (year && day !== null && month !== null) {
        ...
        const calculateDateRangeFromData = (allData) => {
            ...
            allData.forEach(row => {
                const rowDate = getSafeRowDate(row);
                if (rowDate) {
        ...
        const parseSheetDate = (dateStr) => {
            ...
            // Detectar números seriales de Excel/Sheets (ej: 44705) y convertirlos
            // a fecha JS asumiendo epoch de Excel (1899-12-30), usado por Sheets/Excel.
            if (/^\d+(?:\.0+)?$/.test(str)) {
                const serial = Number(str);
                // Excel/Sheets: días desde 1899-12-30
        ...
        function App() {
            ...
            // Calcular producción por artículo (solo INSPECCION)
            const productionByArticle = useMemo(() => {
                const result = {};
                const sampleRow = inspData[0] || {};
                const allColumns = Object.keys(sampleRow);
                const codArtColumn = findColumnName(allColumns, ['Cod. Art.', 'Cod Art', 'CodArt', 'Codigo']);
                const descripcionColumn = findColumnName(allColumns, ['Descripción', 'Descripcion', 'Description']);
                const proveedorHiloColumn = findColumnName(allColumns, ['Proveedor de Hilo', 'Proveedor Hilo', 'ProveedorHilo']);
                const proveedorSpandexColumn = findColumnName(allColumns, ['Proveedor Spandex', 'Proveedor de Spandex', 'ProveedorSpandex']);
                const rolloColumn = findColumnName(allColumns, ['Rollo', 'Nro Rollo', 'No Rollo', 'N° Rollo']);
                const rollosColumn = findColumnName(allColumns, ['Rollos', 'Cantidad Rollos', 'Cant Rollos']);
                const kgColumn = findColumnName(allColumns, ['Peso', 'Peso Kg', 'Peso (kg)', 'Peso kg', 'Kg', 'Kgs', 'Kilos']);
                ...
                inspData.forEach((row, rowIndex) => {
                    const codArt = String(
                        getRowValue(row, codArtColumn, ['Cod. Art.', 'Cod Art', 'CodArt', 'Codigo', 'Código']) ||
                        getRowValueByPatterns(row, ['Cod. Art.', 'Cod Art', 'CodArt', 'Codigo', 'Código']) ||
                        ''
                    ).trim();
                    const descripcion = String(getRowValue(row, descripcionColumn, ['Descripción', 'Descripcion', 'Description']) || '').trim();
                    const proveedorHilo = String(getRowValue(row, proveedorHiloColumn, ['Proveedor de Hilo', 'Proveedor Hilo', 'ProveedorHilo']) || '').trim();
                    const proveedorSpandex = String(getRowValue(row, proveedorSpandexColumn, ['Proveedor Spandex', 'ProveedorSpandex', 'Proveedor de Spandex']) || '').trim();
                    const rolloId = String(getRowValue(row, rolloColumn, ['Rollo', 'Nro Rollo', 'No Rollo', 'N° Rollo']) || '').trim();
                    const pesoKg = parseNumericValue(getRowValue(row, kgColumn, ['Peso', 'Peso Kg', 'Peso (kg)', 'Kg', 'kg', 'Kgs', 'Kilos']));

                    const articleKey = codArt || `SIN_COD_ART_${rowIndex + 1}`;
                    if (!codArt) {
                        console.warn(`Fila ${rowIndex + 1} sin Cod. Art. detectable; se usará una clave temporal.`);
                    }
                    
                    if (!result[articleKey]) {
                        result[articleKey] = {
                            codArt: codArt || articleKey,
                        ...
                        DEFECT_COLUMNS.forEach(def => result[articleKey].defects[def] = 0);
                    }
                    
                    if (rolloId) {
                        result[articleKey].rolloSet.add(rolloId);
                    } else {
                        const fallbackRollos = parseNumericValue(getRowValue(row, rollosColumn, ['Rollos', 'Cantidad'])) || 1;
                        result[articleKey].rollos += fallbackRollos;
                    }

                    result[articleKey].pesoKg += pesoKg;
                    ...
                    DEFECT_COLUMNS.forEach(defect => {
                        ...
                        if (count > 0) {
                            result[articleKey].defects[defect] += count;
                        }
                    });
                    
                    // Actualizar descripción si estaba vacía
                    if (!result[articleKey].descripcion && descripcion) {
                        result[articleKey].descripcion = descripcion;
                    }
                    // Actualizar proveedores si estaban vacíos
                    if (!result[articleKey].proveedorHilo && proveedorHilo) {
                        result[articleKey].proveedorHilo = proveedorHilo;
                    }
                    if (!result[articleKey].proveedorSpandex && proveedorSpandex) {
                        result[articleKey].proveedorSpandex = proveedorSpandex;
                    }
                });
                
                // Calcular los 2 principales defectos para cada artículo
                Object.values(result).forEach(article => {
                    article.rollos += article.rolloSet.size;
                    delete article.rolloSet;
                    article.totalDefectos = Object.values(article.defects).reduce((sum, count) => sum + (count || 0), 0);
                    article.defectosPorKg = article.pesoKg > 0 ? (article.totalDefectos / article.pesoKg) : 0;
            ...
            // Convertir a array ordenado por defecto/kg descendente
            const sortedArticles = useMemo(() => {
                return Object.values(productionByArticle)
                    .sort((a, b) =>
                        (b.pesoKg - a.pesoKg) ||
                        (b.defectosPorKg - a.defectosPorKg) ||
                        (b.totalDefectos - a.totalDefectos) ||
                        (b.rollos - a.rollos)
                    );
            }, [productionByArticle]);

            // Calcular totales generales
            const grandTotalRollos = useMemo(() => {
                return sortedArticles.reduce((sum, article) => sum + article.rollos, 0);
            }, [sortedArticles]);

            const grandTotalKg = useMemo(() => {
                return sortedArticles.reduce((sum, article) => sum + article.pesoKg, 0);
            }, [sortedArticles]);

            const grandTotalDefectos = useMemo(() => {
                return sortedArticles.reduce((sum, article) => sum + article.totalDefectos, 0);
            }, [sortedArticles]);

            const grandDefectosPorKg = useMemo(() => {
                return grandTotalKg > 0 ? (grandTotalDefectos / grandTotalKg) : 0;
            }, [grandTotalDefectos, grandTotalKg]);
            ...
            const handleExportExcel = async () => {
                try {
                    ...
                    // Encabezados
                    const headers = ["Nº", "Cod. Art.", "Descripción", "Kg", "%Part\nkg", "Total Defectos", "Defecto/kg"];
                    ...
                    sortedArticles.forEach((article, index) => {
                        const percentageKg = grandTotalKg > 0 ? ((article.pesoKg / grandTotalKg) * 100).toFixed(1) : 0;
                        const dataRow = worksheet.addRow([
                            ...
                            article.pesoKg,
                            `${percentageKg}%`,
                            article.totalDefectos,
                            article.defectosPorKg
                        ]);
                    ...
                    const footerRow = worksheet.addRow(['', '', 'TOTAL', grandTotalKg, '100%', grandTotalDefectos, grandDefectosPorKg]);
                    ...
                    footerRow.getCell(4).numFmt = '#,##0.0';
                    ...
                    <div className="w-full">
                        {sortedArticles.length > 0 ? (<>
                                    ...
                                    <table className="w-full table-fixed text-[10px] sm:text-xs lg:text-sm">
                                        <thead className="sticky top-0">
                                            <tr className="bg-gray-100">
                                                <th className="px-1.5 sm:px-2 py-2 text-center font-bold text-gray-700 border-b w-[4%]">
                                                    Nº
                                                </th>
                                                <th className="px-1.5 sm:px-2 py-2 text-center font-bold text-gray-700 border-b w-[8%]">
                                                    Cod. Art.
                                                </th>
                                                <th className="px-1.5 sm:px-2 py-2 text-left font-bold text-gray-700 border-b w-[59%] whitespace-nowrap">
                                                    Descripción
                                                </th>
                                                <th className="px-1 sm:px-1.5 py-2 text-center font-bold text-gray-700 border-b bg-emerald-50 w-[7%]">
                                                    Kg
                                                </th>
                                                <th className="px-1 sm:px-1.5 py-2 text-center font-bold text-gray-700 border-b bg-emerald-50 w-[7%] leading-tight">
                                                    <span className="block">%Part</span>
                                                    <span className="block">kg</span>
                                                </th>
                                                <th className="px-1 sm:px-1.5 py-2 text-center font-bold text-gray-700 border-b bg-rose-50 w-[8%] leading-tight">
                                                    <span className="block">Total</span>
                                                    <span className="block">Defectos</span>
                                                </th>
                                                <th className="px-1 sm:px-1.5 py-2 text-center font-bold text-gray-700 border-b bg-cyan-50 w-[7%] leading-tight">
                                                    <span className="block">Defecto/</span>
                                                    <span className="block">kg</span>
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {sortedArticles.map((article, idx) => {
                                                const percentageRollos = grandTotalRollos > 0 ? ((article.rollos / grandTotalRollos) * 100) : 0;
                                                const percentageKg = grandTotalKg > 0 ? ((article.pesoKg / grandTotalKg) * 100) : 0;
                                                const rowBgClass = idx % 2 === 0 ? 'bg-white' : 'bg-gray-50';
                                                
                                                return (
                                                    <tr key={article.codArt} className={`${rowBgClass} hover:bg-blue-50`}>
                                                        <td className="px-1.5 sm:px-2 py-1.5 text-center text-gray-500 border-b">
                                                            {idx + 1}
                                                        </td>
                                                        <td className="px-1.5 sm:px-2 py-1.5 text-center font-mono text-blue-600 border-b whitespace-nowrap truncate">
                                                            {article.codArt}
                                                        </td>
                                                        <td
                                                            className="px-1.5 sm:px-2 py-1.5 text-left text-gray-800 border-b whitespace-nowrap overflow-hidden text-ellipsis truncate"
                                                            title={article.descripcion || '-'}
                                                        >
                                                            {article.descripcion || '-'}
                                                        </td>
                                                        <td className="px-1 sm:px-1.5 py-1.5 text-center font-bold text-emerald-700 border-b bg-emerald-50">
                                                            {formatKg(article.pesoKg)}
                                                        </td>
                                                        <td className="px-1 sm:px-1.5 py-1.5 text-center font-medium text-emerald-700 border-b bg-emerald-50">
                                                            {formatPercent(percentageKg)}
                                                        </td>
                                                        <td className="px-1 sm:px-1.5 py-1.5 text-center font-bold text-rose-700 border-b bg-rose-50">
                                                            {formatInteger(article.totalDefectos)}
                                                        </td>
                                                        <td className="px-1 sm:px-1.5 py-1.5 text-center font-bold text-cyan-700 border-b bg-cyan-50">
                                                            {formatRatio(article.defectosPorKg)}
                                                        </td>
                                                    </tr>
                                                );
                                            })}
                                        ...
                                        <tfoot>
                                            <tr className="bg-gray-200 font-bold sticky bottom-0">
                                                <td colSpan="3" className="px-1.5 sm:px-2 py-2 text-right text-gray-800 border-t">
                                                    TOTAL
                                                </td>
                                                <td className="px-1 sm:px-1.5 py-2 text-center text-emerald-800 border-t bg-emerald-100">
                                                    {formatKg(grandTotalKg)}
                                                </td>
                                                <td className="px-1 sm:px-1.5 py-2 text-center text-emerald-800 border-t bg-emerald-100">
                                                    100%
                                                </td>
                                                <td className="px-1 sm:px-1.5 py-2 text-center text-rose-800 border-t bg-rose-100">
                                                    {formatInteger(grandTotalDefectos)}
                                                </td>
                                                <td className="px-1 sm:px-1.5 py-2 text-center text-cyan-800 border-t bg-cyan-100">
                                                    {formatRatio(grandDefectosPorKg)}
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                            ...
                            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mt-4">
                                <h3 className="text-lg font-semibold text-gray-800 mb-3">Defecto/kg por Artículo</h3>
                                <div className="w-full" style={{ height: '200px' }}>
                                    <BarChart data={sortedArticles} />
                                </div>
                            </div>
        ...
        function BarChart({ data }) {
            ...
            React.useEffect(() => {
                ...
                const defectosPorKg = data.map(article =>
                    parseFloat((article.defectosPorKg || 0).toFixed(2))
                );
                            ...
                            tooltip: {
                                callbacks: {
                                    ...
                                    beforeBody: function(context) {
                                        ...
                                        lines.push(`Rollos: ${formatInteger(article.rollos)}`);
                                        lines.push(`Kg: ${formatKg(article.pesoKg)}`);

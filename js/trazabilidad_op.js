(() => {
    ...
    const buildPartidasGrafica = (records) => {
        ...
        records.forEach(r => {
            ...
            let p = map.get(id);
            if (!p) { p = { cliente: '', tipo: '', aprobada: false, fechaAprob: null, fechaRech: null, eventos: 0 }; map.set(id, p); }
            const cli = normalizeCliente(clean(r.cliente));
            if (cli && !p.cliente) p.cliente = cli;
            if (isAprobada(r)) {
                p.aprobada = true;
                const t = clean(r.tipo_aprobacion).toUpperCase();
    ...
    const buildChartData = (partidas, mode, fCliente, fTipo) => {
    ...
    const ChartModal = ({ records, onClose }) => {
        const [mode, setMode]   = useState('semanas'); // 'meses' | 'semanas'
        const [vMode, setVMode] = useState('count'); // 'count' (123) | 'percent' (%)
        const [fCli, setFCli]   = useState('');      // '' = todos los clientes
        const [fTipo, setFTipo] = useState('');      // '' = todos los tipos de aprobación
        ...
        const tiposAprob = useMemo(
            () => [...new Set(partidas.filter(p => p.estado === 'aprobada' && p.tipo).map(p => p.tipo))].sort(),
            [partidas]
        );
        const chartData = useMemo(() => ({
            ...buildChartData(partidas, mode, fCli, fTipo),
            valueMode: vMode,
            labelAprobadas: fTipo ? `Aprobadas · ${fTipo}` : 'Aprobadas',
                ...
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-[1100px] max-h-[90vh] flex flex-col overflow-hidden"
                    ...
                    <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-[#c8d8bd] bg-[#dfeccd]">
                        ...
                        <div className="flex items-center gap-2">
                            <select value={fTipo} onChange={e => setFTipo(e.target.value)}
                                title="Filtrar la serie de Aprobadas por tipo de aprobación (Rechazadas se muestra completa)"
                                ...
                                {tiposAprob.map(t => <option key={t} value={t}>{t}</option>)}
                            </select>
                            ...
                            <div className="flex rounded-md overflow-hidden border border-[#4f8f62]">
                                <button onClick={() => setVMode('count')} title="Ver número de partidas"
                                    className={`px-3 py-1 text-xs font-bold transition-colors ${vMode==='count' ? 'bg-[#4f8f62] text-white' : 'bg-white text-[#3f7550] hover:bg-[#eef5e8]'}`}>
                                    123
                                </button>
                                <button onClick={() => setVMode('percent')} title="Ver % de participación dentro de cada período"
                                    className={`px-3 py-1 text-xs font-bold transition-colors ${vMode==='percent' ? 'bg-[#4f8f62] text-white' : 'bg-white text-[#3f7550] hover:bg-[#eef5e8]'}`}>
                                    %
                                </button>
                            </div>
                            <div className="flex rounded-md overflow-hidden border border-[#4f8f62]">
                    ...
                    <div className="p-5 flex-1 overflow-auto">
                        <p className="text-[12px] text-[#667466] mb-1">
                            {vMode === 'percent'
                                ? <>Cada punto muestra el <b>% de participación</b> dentro del período (Aprobadas y Rechazadas suman 100%).</>
                                : <>El porcentaje sobre cada período es el <b>nivel de rechazo</b> (partidas rechazadas ÷ partidas auditadas).</>}
                            {fTipo && <> El filtro de tipo aplica solo a las Aprobadas; Rechazadas muestra el total.</>}

...
const { useState, useEffect, useMemo, useCallback, useRef } = React;
        ...
        const DEFECT_COLUMNS = Object.keys(DEFECT_CONFIG);
        ...
        function App() {
            ...
            const defectSummary = useMemo(() => {
                ...
                // Obtener columnas de defectos dinámicamente de los datos
                ...
                if (rawData.length > 0) {
                    ...
                    defectColumnsFromData = availableColumns.filter(col => {
                ...
                // Extraer código del nombre de columna (ej: "1.1 Hueco Con Cordon" -> "1.1")
                const extractCode = (colName) => {
                    const match = colName.match(/^([\d.]+)\s+/);
                    return match ? match[1] : '-';
                };
                
                // Extraer nombre sin código (ej: "1.1 Hueco Con Cordon" -> "Hueco Con Cordon")
                const extractName = (colName) => {
                    const match = colName.match(/^[\d.]+\s+(.+)$/);
                ...
                const summary = defectColumnsFromData
                    .map(defect => ({
                        defecto: defect,
                        nombre: extractName(defect),
                        valor: DEFECT_CONFIG[defect]?.valor || 'UND',
                        codigo: extractCode(defect),

...
const { useState, useEffect, useMemo, useCallback } = React;
        ...
        const getSafeRowDate = (row) => {
            const rawYear = String(row['AÃ±o'] || row['Anio'] || row['Year'] || '').trim();
            const fallbackYear = /^\d{4}$/.test(rawYear) ? parseInt(rawYear, 10) : null;
            const rawDate = row['DÃ­a'] || row['Dia'] || row['Day'] || row['Fecha'] || row['Date'] || '';

            const parsed = parseSheetDate(rawDate, fallbackYear);
            if (parsed) return parsed;

            const rawDay = row['DÃ­a'] || row['Dia'] || row['Day'] || '';
        ...
        const getComparableRowDate = (row) => {
            const rawYear = String(row['Año'] || row['AÃ±o'] || row['Anio'] || row['Year'] || '').trim();
            const fallbackYear = /^\d{4}$/.test(rawYear) ? parseInt(rawYear, 10) : null;
            const rawDate = row['Día'] || row['DÃ­a'] || row['Dia'] || row['Day'] || row['Fecha'] || row['Date'] || '';

            const parsed = parseSheetDate(rawDate, fallbackYear);
            if (parsed) return parsed;

            const rawDay = row['Día'] || row['DÃ­a'] || row['Dia'] || row['Day'] || '';
            const rawMonth = row['Mes'] || row['Month'] || '';
            const day = parseInt(String(rawDay).replace(/[^0-9]/g, ''), 10);
        ...
        function App() {
            ...
            const availableDates = useMemo(() => {
                ...
                rawData.forEach(row => {
                    ...
                    const rowYear = String(row['AÃ±o'] || row['Anio'] || row['Year'] || '').trim() || (rowDate ? String(rowDate.year) : '');
        ...
        function BarChart({ data, periodoType }) {
            ...
            React.useEffect(() => {
                ...
                const metaData = data.map(item => item.metaRollos || 0);
                        ...
                        chart.data.datasets.forEach((dataset, datasetIndex) => {
                            ...
                            meta.data.forEach((element, index) => {
                                ...
                                if (dataset.label === 'Rollos') {
                                    ...
                                    const metaValue = data[index]?.metaRollos || 0;
                                    const percent = metaValue > 0 ? Math.round((value / metaValue) * 100) : 0;
                                    ctx.fillStyle = 'rgba(30, 64, 175, 1)';
                                    ctx.fillText(value.toLocaleString('es-ES'), element.x, element.y - 24);
                                    ctx.font = 'bold 11px Arial';
                                    ctx.fillText(`${percent}%`, element.x, element.y - 11);

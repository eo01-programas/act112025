(() => {
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3">
            ...
            <div class="print-card bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex items-center justify-between transition-all-300 hover:shadow-md">
                <div>
                    ...
                    <div class="flex items-baseline gap-2">
                        <h3 id="kpi-neto-bien" class="text-3xl font-extrabold text-[#3f7550]">73</h3>
                        <span id="pct-bien" class="text-xs font-bold bg-[#eef5e8] text-[#3f7550] px-1.5 py-0.5 rounded-md">72%</span>
                    </div>
            ...
            <div class="print-card bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex items-center justify-between transition-all-300 hover:shadow-md">
                <div>
                    ...
                    <h3 id="kpi-aprobadas-totales" class="text-3xl font-extrabold text-[#4c8ca8]">160</h3>
                    <p class="text-[11px] font-medium text-[#4c8ca8]/80 mt-1">Aprob. + Tolerancia + Autoriz.</p>
                </div>
            ...
            <div class="print-card bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex items-center justify-between transition-all-300 hover:shadow-md">
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Volumen de Partidas</p>
                    <h3 id="kpi-partidas-totales" class="text-3xl font-extrabold text-slate-900">1166</h3>
                    <p class="text-[11px] font-medium text-slate-500 mt-1">Total de lotes auditados</p>
                </div>
            ...
            <div class="print-card bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex items-center justify-between transition-all-300 hover:shadow-md">
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Cantidad Rechazos</p>
                    <div class="flex items-baseline gap-2">
                        <h3 id="kpi-rechazos" class="text-3xl font-extrabold text-[#b65b5b]">181</h3>
                        <span id="pct-rechazos" class="text-xs font-bold bg-[#f6e8e8] text-[#8a2f2f] px-1.5 py-0.5 rounded-md">16%</span>
                    </div>
                    <p class="text-[11px] font-medium text-[#b65b5b]/80 mt-1">Lotes con desviación técnica</p>
                </div>
        ...
        <main class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="print-card bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col h-[350px]">
                <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                    ...
                    <span class="text-[10px] bg-[#eef5e8] text-[#3f7550] font-bold px-2 py-0.5 rounded-full uppercase">Cálculo Neto</span>
                </div>
    ...
    // ───────────────────────── Lógica de la vista ─────────────────────────
        ...
        function getExcelField(row, targets) {
            const normalizedTargets = targets.map(t => normalizeString(t));
        ...
        function mapearRegistros(records) {
            return (records || []).map(mapearRegistro);
        }
            ...
            if (!forzar) {
                try {
                    ...
                    if (raw) {
                        ...
                        if (Array.isArray(parsed) && parsed.length) aplicarRegistros(parsed);
                    }
                } catch (e) { /* ignore */ }
            }
            try {
        ...
        function actualizarVisualizacion() {
            ...
            // MODIFICACIÓN CRÍTICA: Se calcula la sumatoria de rechazos únicamente para las partidas que pertenecen a la Cuenta de tipo_aprobacion
            // Es decir, solo contamos los rechazos de las partidas con estado APROBADO, APROBADO C/TOLERANCIA o APROBADO C/AUTORIZACION (excluyendo los RECHAZADOS totales)
            const totalRechazos = dataset
                .filter(item => item.tipo === "APROBADO" || item.tipo === "APROBADO C/TOLERANCIA" || item.tipo === "APROBADO C/AUTORIZACION")
                .reduce((acc, row) => acc + (row.rechazos || 0), 0);

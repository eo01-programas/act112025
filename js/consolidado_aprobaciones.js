(() => {
                ...
                <section class="shrink-0 glass-card rounded-2xl p-4 shadow-sm no-print">
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <button onclick="abrirTendencia()" title="Tendencia de aprobaciones y rechazos" class="shrink-0 flex items-center justify-center w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 transition-all-300">
    ...
    <!-- MODAL DE TENDENCIA: aprobaciones vs rechazos por período (js/chart_tendencia.js) -->
    <div id="tendencia-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 no-print" onclick="cerrarTendencia()">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[1100px] max-h-[90vh] flex flex-col overflow-hidden" onclick="event.stopPropagation()">
            <div class="flex items-center justify-between gap-3 px-5 py-3 border-b border-[#c8d8bd] bg-[#dfeccd]">
                <div class="min-w-0">
                    <h2 class="text-base font-extrabold text-[#3f7550]">Tendencia de aprobaciones y rechazos</h2>
                    <p id="tendencia-filtros" class="text-[11px] font-semibold text-[#667466] truncate"></p>
                </div>
                <div class="flex items-center gap-2 shrink-0">
                    <div class="inline-flex bg-white p-1 rounded-xl border border-[#c8d8bd]">
                        <button id="btn-tend-semana" onclick="setTendenciaModo('semana')" class="flex-1 py-1.5 px-4 text-xs font-bold rounded-lg transition-all-300 text-slate-600 hover:text-slate-900">Semana</button>
                        <button id="btn-tend-mes" onclick="setTendenciaModo('mes')" class="flex-1 py-1.5 px-4 text-xs font-bold rounded-lg transition-all-300 text-slate-600 hover:text-slate-900">Mes</button>
                    </div>
                    <button onclick="cerrarTendencia()" title="Cerrar" class="flex items-center justify-center w-8 h-8 rounded-md bg-white border border-[#c8d8bd] hover:bg-rose-50 text-[#3f7550] text-lg">✕</button>
                </div>
            </div>
            <div class="p-5 flex-1 overflow-auto">
                <p class="text-[12px] text-[#667466] mb-2">
                    El porcentaje sobre cada período es el <b>nivel de rechazo</b> (partidas rechazadas ÷ partidas auditadas).
                    Pase el cursor sobre los puntos para ver el detalle por cliente.
                </p>
                <div class="relative" style="height: 55vh; min-height: 340px;">
                    <canvas id="chart-tendencia"></canvas>
                    <div id="tendencia-empty" class="hidden absolute inset-0 flex items-center justify-center text-[#667466]">
    ...
    // ───────────────────────── Lógica de la vista ─────────────────────────
        ...
        // ─── Modal de tendencia: aprobaciones vs rechazos por período ───
        // Reusa activeData (partidas únicas ya deduplicadas) y respeta los filtros
        // de año, cliente y artículo. Ignora mes/semana porque la gráfica recorre
        // los períodos. El dibujo vive en js/chart_tendencia.js (compartido con
        // el modal de Trazabilidad OP-Partida).
        let tendenciaChart = null;
        let tendenciaModo  = 'semana'; // 'semana' | 'mes'
        const MESES_ORDEN = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];

        function construirDatosTendencia() {
            let ds = [...activeData];
            if (state.ano !== 'Todos')      ds = ds.filter(it => it.anio.toString() === state.ano);
            if (state.cliente !== 'Todos')  ds = ds.filter(it => it.cliente === state.cliente);
            if (state.articulo !== 'Todos') ds = ds.filter(it => it.articulo === state.articulo);

            // Con varios años en juego, la etiqueta lleva el año para no mezclar períodos
            const multiAnio = new Set(ds.map(it => it.anio)).size > 1;

            const buckets = new Map(); // clave cronológica (anio*100 + índice) → acumulador
            ds.forEach(it => {
                let key, label;
                if (tendenciaModo === 'mes') {
                    const mi = MESES_ORDEN.indexOf(it.mes);
                    if (mi < 0) return;
                    key = it.anio * 100 + mi;
                    label = multiAnio ? `${it.mes} ${String(it.anio).slice(2)}` : it.mes;
                } else {
                    const wn = parseInt(it.semana.replace(/\D/g, ''), 10);
                    if (isNaN(wn)) return;
                    key = it.anio * 100 + wn;
                    label = multiAnio ? `S${wn} '${String(it.anio).slice(2)}` : `SEM ${wn}`;
                }
                let b = buckets.get(key);
                if (!b) { b = { label, aprob: 0, rech: 0, detA: new Map(), detR: new Map() }; buckets.set(key, b); }
                if (it.tipo === 'RECHAZADO') {
                    b.rech++;
                    b.detR.set(it.cliente, (b.detR.get(it.cliente) || 0) + 1);
                } else {
                    b.aprob++;
                    b.detA.set(it.cliente, (b.detA.get(it.cliente) || 0) + 1);
                }
            });

            // Últimos 12 períodos con datos, en orden cronológico
            const orden = Array.from(buckets.entries()).sort((a, b) => a[0] - b[0]).slice(-12).map(e => e[1]);
            const toSorted = (m) => Array.from(m.entries()).sort((a, b) => b[1] - a[1]).map(([label, value]) => ({ label, value }));
            return {
                labels:     orden.map(b => b.label),
                aprobadas:  orden.map(b => b.aprob),
                rechazadas: orden.map(b => b.rech),
                detalles:   orden.map(b => ({ aprobadas: toSorted(b.detA), rechazadas: toSorted(b.detR) })),
                hasData:    orden.length > 0,
            };
        }

        function renderTendencia() {
            const canvas = document.getElementById('chart-tendencia');
            if (!canvas || !window.Chart || !window.ChartTendencia) return;
            const data = construirDatosTendencia();
            if (tendenciaChart) { tendenciaChart.destroy(); tendenciaChart = null; }
            const vacio = document.getElementById('tendencia-empty');
            if (vacio) vacio.classList.toggle('hidden', data.hasData);
            if (data.hasData) tendenciaChart = window.ChartTendencia.render(canvas, data);
        }

        function setTendenciaModo(modo) {
            tendenciaModo = modo;
            const on  = "flex-1 py-1.5 px-4 text-xs font-bold rounded-lg transition-all-300 bg-[#4f8f62] text-white shadow-sm";
            const off = "flex-1 py-1.5 px-4 text-xs font-bold rounded-lg transition-all-300 text-slate-600 hover:text-slate-900";
            const bS = document.getElementById('btn-tend-semana');
            const bM = document.getElementById('btn-tend-mes');
            if (bS) bS.className = modo === 'semana' ? on : off;
            if (bM) bM.className = modo === 'mes' ? on : off;
            renderTendencia();
        }

        function abrirTendencia() {
            // Subtítulo con los filtros que la gráfica sí respeta
            const partes = [];
            partes.push(state.ano === 'Todos' ? 'Todos los años' : `Año ${state.ano}`);
            partes.push(state.cliente === 'Todos' ? 'Todos los clientes' : state.cliente);
            if (state.articulo !== 'Todos') partes.push(`Art. ${state.articulo}`);
            safeSetText('tendencia-filtros', partes.join(' · '));
            safeRemoveClass('tendencia-modal', 'hidden');
            // Siempre abre en Semana (el usuario puede cambiar a Mes en el modal)
            setTendenciaModo('semana');
        }

        function cerrarTendencia() {
            safeAddClass('tendencia-modal', 'hidden');
            if (tendenciaChart) { tendenciaChart.destroy(); tendenciaChart = null; }
        }

        // Cerrar con tecla Escape (guard en window: el router puede reinyectar el script)
        if (window.__caTendenciaEsc) window.removeEventListener('keydown', window.__caTendenciaEsc);
        window.__caTendenciaEsc = (e) => {
            const modal = document.getElementById('tendencia-modal');
            if (e.key === 'Escape' && modal && !modal.classList.contains('hidden')) cerrarTendencia();
        };
        window.addEventListener('keydown', window.__caTendenciaEsc);
    ...
    window.abrirTendencia = abrirTendencia;
    window.cerrarTendencia = cerrarTendencia;
    window.setTendenciaModo = setTendenciaModo;
